...
<thinking>OCR识别出"1850"，但根据上下文瑞幸咖啡的消费通常在10-50元范围，"1850"大概率是OCR漏了小数点，实际应为18.50元。修正为18.50。</thinking>
...
2. amountYuan: **这是最关键的一项，必须仔细识别！**
   ...
   - OCR纠错：如果金额看起来不合理（如快餐/咖啡场景出现"1850"），根据商户类型推断合理范围并修正（"1850"→18.50）
...
export async function parseWithAI(text: string, apiKey: string): Promise<AiParsed> {
  ...
  // Priority 1: Extract from <result> tag (chain-of-thought format)
  const resultMatch = content.match(/<result>\s*(\{[\s\S]*?\})\s*<\/result>/);
  if (resultMatch) jsonStr = resultMatch[1];

  // Priority 2: Extract bare JSON object (fallback for models that ignore tags)
  if (!resultMatch) {
